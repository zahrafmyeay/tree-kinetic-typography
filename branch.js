...
function setup() {
  ...
  let initialBranch = new Branch(width / 2, height, -PI / 2, initialBranchLength, null, 0); 
...
function draw() {
  ...
  if (wordIndex >= leftWords.length + middleWords.length + rightWords.length + centerWords.length) {
    falling = true; 
  }
...
class Branch {
  ...
  branchOut() {
    ...
    if (wordIndex < leftWords.length) {
      newBranch = new Branch(newX, newY, this.angle - angleOffset, branchLength, leftWords[wordIndex], newLevel); 
      leftBranchCount++; 
      ...
      newBranch = new Branch(xStart, yStart, this.angle + angleOffset, branchLength, middleWords[wordIndex - leftWords.length], newLevel);
      middleBranchCount++;
      ...
      newBranch = new Branch(xStart, yStart, rightAngle, branchLength, rightWords[wordIndex - leftWords.length - middleWords.length], newLevel); 
      rightBranchCount++;
      ...
      newBranch = new Branch(xStart, yStart, centerAngle, branchLength, centerWords[wordIndex - leftWords.length - middleWords.length - rightWords.length], newLevel); 
      centerBranchCount++; 
    }
